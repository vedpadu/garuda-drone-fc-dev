...
void HAL_SPI_TxRxCpltCallback(SPI_HandleTypeDef *hspi){
	if(hspi == hspi_bmi270){
		...
		accel_pre_filt[0] = (float32_t)bmi270_lsb_to_mps2((int16_t)accelXBin, (float)2.0, 16);
		accel_pre_filt[1] = (float32_t)bmi270_lsb_to_mps2((int16_t)accelYBin, (float)2.0, 16);
		accel_pre_filt[2] = (float32_t)bmi270_lsb_to_mps2((int16_t)accelZBin, (float)2.0, 16);

		gyro_pre_filt[0] = (float32_t)bmi270_lsb_to_dps(gyroXSigned, (float)2000.0, 16) * M_PI / 180.0;
		gyro_pre_filt[1] = (float32_t)bmi270_lsb_to_dps((int16_t)gyroYBin, (float)2000.0, 16) * M_PI / 180.0;
		gyro_pre_filt[2] = (float32_t)bmi270_lsb_to_dps(gyroZSigned, (float)2000.0, 16) * M_PI / 180.0;
